...
abstract class PersonalNumberStorageAbs{
	...
	public abstract String searchPersonalInfo(String jumiNum);
}
/*
DTO(Data Transfer Object) : 데이터를 저장하고 전송하기 위한 용도의
	객체를 말한다. VO(Value Object)로 표현하기도 한다.
	해당프로그램에서는 한 사람의 정보를 저장하기 위한 용도로 정의한다.
 */
...
class PersonalNumberStorageExt extends PersonalNumberStorageAbs{
	...
	@Override
	public String searchPersonalInfo(String jumiNum)
	{
		for(int i =0 ; i<numOfPerInfo ;i++) {
			/*
			멤버변수를 private로 선언했으므로 외부에서는 접근할 수 없다.
			이럴때 getter메서드를 통해 접근하여 값을 얻어올 수 있다.
			 */
			if(jumiNum.compareTo(personalArr[i].getJuminNum())==0) {
