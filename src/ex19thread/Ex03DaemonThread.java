...
종속쓰레드(Daemon Thread)
	...
	(메인메서드도 하나의 독립쓰레드 이므로 다른 독립쓰레드가 남아 있다면
	그 독립쓰레드가 종료될때까지는 프로그램은 종료된것이 아니다.)
...
public class Ex03DaemonThread
{
	...
	public static void main(String[] args)
	{
		...
		nt2.NotThreadMethod();
		//위는 1번객체가 작업을 끝내야지만 2번 객체가 실행된다. 즉 순차실행된다.
		
		//첫번째 독립쓰레드 생성.
		YesThread yt1 = new YesThread("1st 쓰레드"); //이름부여 방법 1번
		//yt1.setName("첫번째쓰레드");  //이름부여 방법 2번
		yt1.start();

		//두번째 독립쓰레드 생성
		YesThread yt2 = new YesThread(); // 이름을 지정하지 않아 자동으로 부여됨.
		...
		dt.setName("난 데몬쓰레드");
		dt.setDaemon(true); //독립쓰레드를 종속쓰레드로 만들어 준다.
		dt.start();
		
		//Thread 클래스의 정적 메서드를 통해 여러가지 정보확인
