...
/*
	...
	:예외 발생과 상관없이 try문에 진입했을때 반드시 실행해야 할
	코드가 있는 경우 기술하는 블럭이다.
...
{
	/*
	런타임 예외가 발생되는 메소드. 예외 발생시 무시하기 위해 throws 하고 있다.
	하지만 이와같은 예외는 어디서든 반드시 예외처리 해야한다.
	 */
	...
	static void tryCatchFinally() {
		...
		try {
			...
			/*
			try문 진입후  예외 발생 여부에 상관없이 해당 구문은 실행된다.
	...
	{
		/*
		만약 이 지점에서도 예외던지기(throws exception)를 하면 프로그램은 
		즉시 중지된다.(에러) 즉 반드시 처리해할 예외가 존재한다.
		 */
		try {
			runtime();
		}
