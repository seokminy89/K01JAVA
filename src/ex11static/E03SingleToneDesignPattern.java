...
/*
싱글톤 디자인 패턴
	:클래스를 설계하는 디자인패턴의 하나라로써 하나의 인스턴스
	즉 하나의 메모리를 생성해 이를 공유해서 사용하고자 할때
	쓰는 패턴이다.
작성방법
	1.생성자의 접근지정자를 private으로 정의한다.
	2.클래서 외부에서는 생성자를 통해 접근할 수 ㅇ벗으므로
		new를 통해 객체생성은 할 수 없다.
	3.클래스 내부에 정적 메서드로 해당 클래스의 객체를
		반환하도록 정의한다. => getInstance()
	4.위 방법을 통해 객체를 생서하면 메모리에 딱 하나의 
		객체만 만들어지게 된다.
 */
//일반적인 클래스
class NoSingleTone{
...
class SingleTone{
	int shareVar;
	/*
	JVM(자바가상머신)에 의해 프로그램이 시작될 때 정적 변수는
	미리 메서드 영역에 생성되어 사용할 준비를 마치게 된다.
	 */
	...
	/*
	정적메소드로 선언된 getInstance() 메서드에서 해당 객체를
	외부로 반환한다.
	 */
