...
public class E01WrapperClass2
{
	...
	public static void main(String[] args)
	{
		...
		System.out.println("10+strNumber="+(10+strNumber));//숫자 + 문자열(1000)
		...
		String floaString = "3.14";
		/*
		실수형태의 문자열을 실수로 변경할때는 parseInt()를 사용할 수 없다.
		위와 동일한 예외가 발생한다.
		 */
		//System.out.println(Integer.parseInt(floaString));//예외발생
		System.out.println("실수형(float)형으로 변경:"+ 
				Float.parseFloat(floaString));
		System.out.println("실수형(Double)형으로 변경:"+ 
				Double.parseDouble(floaString));
		
		/*
		Character 클래스의 주요 메소드
		
		codePointAt() : 문자열에서 index에 해당하는 한 문자의 아스키 코드 값을 
			반환한다.
		 */
		...
		System.out.println("총공백수="+whiteCount);//6
	}
}
