...
public class R01if
{
	...
	public static void main(String[] args)
	{
		...
		int num1 = 10;
		int num2 = 20;
		if(num1<num2) {// 조건이 true 일 때 해당 구문 실행됨.
			System.out.println("num1이 num2보다 작다.");
		}
		
		
		//위문장을 if~else문으로 변경하면...
		if(num1>num2) {
		...
		/*
		주의] if~else문에서 else부분에는 조건이 들어가지 않는다.
			항상 if문에만 들어가야 한다.
		 */
		...
		70~80점 미만 : C학점
		나머지는 F학점 
		: 점수의 구간같은 경운에는 이상, 이하에 해당하는
		두 조건을 묶어주는게 안전하다.
		
		아래와 같이 구간을 조건식으로 사용하면 위치에 상관없이
		해당 조건식은 사용이 가능하므로 좀더 유연하고 안전한 코딩이
		가능하다.
		 */
		int jumsu=79;
		if(jumsu >= 90 ) {
...
//		}
		...
		/*
		만약 3을 입력했다면 break가 없는 경우 아래의 모든 문장이 출려된다.
