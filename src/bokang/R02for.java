...
public class R02for
{
	...
	public static void main(String[] args)
	{
		 ...
		 */
		//시나리오] 위에서 만든 수열 문제를 do~while문으로 변경하시오.
		...
		System.out.println("1~10가지의 합:"+sum);
		 ...
		 */
		//주사위1(1~6까지)
		System.out.println("for문으로 주사위 문제 구현하기");
		for(int i=1 ; i <=6 ; i++) {
			//주사위2(1~6까지)
			for(int j=1 ; j <=6 ; j++) {
		...
		//연습문제] 앞의 주사위 문제를 while 문으로 변경.
		System.out.println("while문으로 주사위 문제 구현하기");
		int i = 1;
		//주사위 1은 i를 통해 반복
		while(i <= 6) {
			int j = 1;
		...
		//연습문제] 앞의 주사위 문제를 do~while 문으로 변경.
		System.out.println("do~while문으로 주사위 문제 구현하기");
		int k = 1;
		do {
			int l = 1;
			do {
		...
		for(int n=0 ; n<=10; n++) {
			for(int m=0; m <=10 ; m++) {
				if((2*n)+(4*m)==12) {
					//System.out.printf("2n+4m=12를 만족하는 n,m:%d, %d%n",n,m);
					System.out.println("2n+4m=12를 만족하는 n,m:"+n+", "+m);
				}
			}
		}
	}
