...
public class E04DoWhile {
	...
	public static void main(String[] args) throws IOException{
		...
		do { //조건 확인 없이 무조건 1번 실행
			sum += i; //증가하는 변수 i를 sum에 누적해서 더함
			i++; //i를 증가시킴(반복문 탈출을 위해서)
		...
		int j=1; // 반복을 위한 변수
		do { //조건 검사없이 진입
			if(j%4==0 || j%7==0) {//4 또는 7의 배수 일때...
				//System.out.println("j="+j); // 배수 확인용 출력
				total += j; //증가하는 j의 값을 total에 누적해서 더함.
			}
			j++; // 탈출을 위함
		...
		/*
		 X를 입력한 경우
		    : !(false || true) => !(true) => false를 반환
		    do~while문을 탈출한다.
		 a를 입력한 경우
		    : !(false || false) => !(false) => true를 반환
		    do~while문을 반복한다.
		 */
	}
}
