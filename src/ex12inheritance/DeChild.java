...
public class DeChild extends DeParent {
	...
	//자식 클래스에서 확장한 메서드(= 부모클래스에는 없는 메서드)
	public void study() {
	...
	/*
	 오버라이딩 아님.
	 : 부모클래스에서 private 멤버로 선언하였기 때문에 자식쪽에서는
	 보이지 않아 오버라이딩의 대상이 될 수 없다.
	...
	그보다 넓으면 가능하다.
	-부모에서 default로 선언하면 자식에서는 default, 혹은 그보다 넓으면(public) 
	오버라이딩이 가능하다.
	 */
	@Override
	public String sleep()
	...
	public void printParent()
	{
		...
		-super.멤버 : 부모클래스의 멤버메서드 혹은 변수를 접근할때 사용.
		 */
		super.printParent();
	...
	클래스 외부(메서드영역)에 로드되므로 오버라이딩 되지 않고 호출할때
	각 클래스명을 통해 호출해야 한다. 
	 */
