...
class C extends B{
	...
	모두 가려지게되므로 C클래스에 정의된 메서드 하나만 남게 된다.
...
public class E07RideAndLoad01
{
	...
	public static void main(String[] args)
	{
		...
		호출된다.
		즉 아래 부분은 모두 C클래스에 생성된 rideMethod() 메서드가 호출된다.
		 */
		...
		ref2.loadMethod();
		ref2.loadMethod(1);
		//ref2.loadMethod(1.1);//-> C 클래스 영역은 접근불가
		
		
		//ref1는 A타입이므로 접근범위가 A까지로 제한된다.
		ref1.loadMethod();
		//ref1.loadMethod(1);//-> B 클래스 영역은 접근불가
		//ref1.loadMethod(1.1);//-> C 클래스 영역은접근불가
		...
		C refAddr1 = new C(); //가능
		B refAddr2 = refAddr1; //refAddr1이 C타입이기 때문에 가능 B타입=>C타입
		A refAddr3 = refAddr1; //가능 A타입(부모) => C타입(자식) 
		
		/*
		참조변수 refId1이 C객체를 참조하고 있으므로 마치 refId1이 C타입이라고
		착각할 수 있으나, 처음부터 refId는 A타입의 참조변수이다.
		자식으로 부모를 참조하는것은 불가능하므로 아래 2개의 문장은
		에러가 발생한다.
		 */
		A refId1 = new C(); //가능
		//B refId2 = refId1; //refId1이 A타입이기 때문에 B(자식)=>A(부모)타입 불가능 에러발생
		//C refId3 = refId1;//refId11이 A타입이기 때문에 C(자식)=>A(부모)타입 불가능 에러발생
		
		/*
		위에서 에러가 난 부분은 아래처럼 다운캐스팅(강제적형변환) 하면
